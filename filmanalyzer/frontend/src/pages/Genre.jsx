...
export default function Test() {
  ...
  async function searchGross(name) {
    ...
    if (searchBy == "CASTNAME") {
      res = await executeQuery(
        'SELECT GROSSINGWORLD, RELEASEDATE FROM (SELECT MOVIEID FROM "JONATHAN.CUNNING".WORKSON INNER JOIN (SELECT CASTID FROM "JONATHAN.CUNNING".CAST WHERE lower ( CASTNAME )= :name ) A ON "JONATHAN.CUNNING".WORKSON.CASTID = A.CASTID) B INNER JOIN "JONATHAN.CUNNING".MOVIE ON B.MOVIEID = "JONATHAN.CUNNING".MOVIE.MOVIEID',
        [name]
      );
    } else {
      res = await executeQuery(
        'SELECT GROSSINGWORLD, RELEASEDATE FROM (SELECT MOVIEID FROM "JONATHAN.CUNNING".WORKSON WHERE CASTID = :name) B INNER JOIN "JONATHAN.CUNNING".MOVIE ON B.MOVIEID = "JONATHAN.CUNNING".MOVIE.MOVIEID',
        [parseInt(name)]
      );
    }
    return res;
